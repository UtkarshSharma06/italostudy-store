...
import {
    ...
    Package, Monitor, Truck, LogIn, SlidersHorizontal,
    Grid3X3, List, ArrowUpDown, ChevronRight, Filter
} from 'lucide-react';
...
export default function AllProducts({ isMobileView = false }: { isMobileView?: boolean }) {
        ...
        <div onClick={() => navigate(`/${product.slug}`)}
            ...
            <div className="p-3 flex flex-col gap-2 flex-1">
                ...
                <div className="flex items-baseline gap-1.5 mt-auto">
                    <span className="text-base font-black text-[#0f172a]">€{product.price}</span>
                    {product.discount_price && (
                        <span className="text-xs text-slate-400 line-through">€{product.discount_price}</span>
                    )}
                </div>
                <button onClick={e => onAddToCart(product, e)}
                    ...
                    {isAdded ? <><Check className="w-3 h-3" /> Added!</> : <><ShoppingBag className="w-3 h-3" /> Add to Cart</>}
                </button>
            </div>
        </div>
    );
